...
export function AiPostAssistant({ isOpen, onClose, onApply, community }: { isOpen: boolean; onClose: () => void; onApply: (post: GeneratedPost) => void; community: string }) {
  ...
  return <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex justify-end">
    <aside className="w-full sm:w-1/2 max-w-2xl h-full overflow-y-auto bg-[--surface] border-l border-[--border] shadow-2xl p-5 sm:p-7 animate-in slide-in-from-right duration-200">
